...
function parse (text) {
  ...
  while (index < lines.length) {
    ...
    if (match = line.match(HEADER)) {
      ...
      while (index < lines.length) {
        const match = current().match(QUOTE)
        if (!match) break
        const [_, item] = match
        items.push(item)
        index++
    ...
    } else if (match = line.match(PROMPT)) {
      const [_, href, rawContent] = match
      const content = rawContent || href

      tokens.push({ type: 'prompt', content, href })
    } else if (match = line.match(HR)) {
      tokens.push({ type: 'hr' })
    } else {
